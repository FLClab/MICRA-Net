...
def process_dir(top_dir, opts):
    ...
    if (".git" in path_top_dir.parts):
        print(f"SKIPPING {path_top_dir}")
        return
    ...
    for entry in sorted_entries:
        ...
        if entry_type in {"folder", "folder-shortcut"}:
            index_file.write(f"""
                <tr class="file">
                    <td></td>
                    <td>
                        <a href="{entry_path}index.html">
...
if __name__ == "__main__":
    ...
    #######################################################
    #######################################################
    # Creates an index file within all folders recursively
    #######################################################
    #######################################################
    ...
    Config = namedtuple("config", ["filter", "recursive", "top_dir", "verbose"])
    config = Config(filter=None, recursive=True, top_dir=Path(top_dir), verbose=False)
